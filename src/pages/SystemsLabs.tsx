...
import { decisionLabDefinitions } from './DecisionLab';

export default function SystemsLabs() {
  const { user } = useAuth();
  const accountScope = user?.id ?? 'guest';
  const attempts = loadSystemsLabAttempts(accountScope);
  const { decisionReceipts } = useLearningEvidence();

  return (
    <PageShell wide>
      <PageHeader
        eyebrow="Labs"
        title="Predict first. Decide from evidence."
      ...
      <section className="mb-12" aria-labelledby="decision-labs">
        <div className="mb-5 max-w-3xl">
          <h2 id="decision-labs" className="text-xl font-semibold tracking-tight text-white">
            Decision labs
          </h2>
          <p className="mt-2 text-sm leading-relaxed text-white/55">
            Freeze an expectation, reveal deterministic arithmetic, then record a mitigation,
            counterfactual, and verification metric.
          </p>
        </div>
        <div className="overflow-hidden rounded-xl border border-white/[0.08]">
          {decisionLabDefinitions.map((lab, index) => {
            const receipts = decisionReceipts.filter((receipt) => receipt.labId === lab.id);
            const verified = receipts.some((receipt) => receipt.evidenceState === 'verified');
            return (
              <Link
                key={lab.id}
                to={`/labs/decision/${lab.id}`}
                className={`group block bg-black px-5 py-5 transition-colors hover:bg-white/[0.03] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-white/50 sm:px-7 ${
                ...
                <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <Calculator className="h-4 w-4 text-sky-300" aria-hidden="true" />
                  ...
                  <div className="flex items-center justify-between gap-6 sm:min-w-36">
                    <span className="font-mono text-xs text-white/50">
                      {lab.estimatedMinutes} min · {receipts.length} receipt
                      {receipts.length === 1 ? '' : 's'}
                    </span>
      ...
      <section aria-labelledby="systems-simulations">
        <div className="mb-5 max-w-3xl">
          <h2 id="systems-simulations" className="text-xl font-semibold tracking-tight text-white">
            Systems simulations
          </h2>
          <p className="mt-2 text-sm leading-relaxed text-white/55">
