      ...
      {conceptNotes.map(n => (
        <Card key={n.id} className="flex items-start justify-between gap-3 p-3">
          <div className="min-w-0">
            ...
            <div className="mt-1 text-[11px] text-slate-500">{new Date(n.updatedAt).toLocaleDateString()}</div>
          </div>
          <button onClick={() => deleteNote(n.id)} aria-label="Delete note" className="shrink-0 text-slate-500 hover:text-rose-400">
