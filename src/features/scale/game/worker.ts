import { freshCompany } from './model';
...
let suspended = false;
let hiddenAt = 0;
self.onmessage = ({ data }: MessageEvent<Command | { type: 'visibility'; hidden: boolean }>) => {
  try {
    if (data.type === 'visibility') {
      ...
      runtime = new Runtime(data.company ?? freshCompany(data.scenario));
...
setInterval(() => {
  ...
  try {
    runtime.step(100);
    if (++counter % 3 === 0) self.postMessage({ type: 'view', view: runtime.view() });
  } catch (error) {
    self.postMessage({
      type: 'error',
      message: error instanceof Error ? error.message : 'Simulation failed',
