...
import { ToolLearning, FailureLearning } from './LearningLinks';
import { ToolkitCatalog } from './ToolkitCatalog';
import { InfrastructureMetrics } from './InfrastructureMetrics';
import { serviceHealthy } from './game/health';
import { prerequisite, projectHours } from './game/toolkit';
import { upgrade } from './game/model';
import { challengeStopped, diagnosis, targetUsers } from './game/challenge';
...
import type { Decision, Workload } from './game/model';
...
function useSimulation(saveKey: string, scenario: Workload) {
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      try {
        originalRaw = localStorage.getItem(saveKey);
        company = parseSave(originalRaw);
        protectSave = !!originalRaw && !company;
        if (protectSave)
          setStorage('Unreadable save preserved; this session will not overwrite it');
      } catch {
        setStorage('Storage unavailable. Keep this tab open to retain progress.');
      }
      unlockSave.current = () => {
        protectSave = false;
        try {
      ...
      w.onerror = () =>
        setError('The simulation could not start. Reload to restore your last saved company.');
      w.postMessage({ type: 'init', company, scenario });
      const save = () => {
        if (!latest.current || protectSave) return;
        try {
          if (localStorage.getItem(saveKey) !== originalRaw) {
            protectSave = true;
    ...
    // Defer acquisition past React StrictMode's setup/cleanup replay.
    queueMicrotask(() => {
      if (!alive) return;
      if (navigator.locks) {
        void navigator.locks
          .request(`${saveKey}:writer`, { ifAvailable: true }, async (lock) => {
            if (!alive) return;
            if (!lock) {
              setError(
          ...
          .catch(() => {
            if (alive) setError('Could not acquire the company save lock. Reload to try again.');
          });
      } else start();
    });
    return () => {
...
function TrafficSlider({ value, change }: { value: number; change: (value: number) => void }) {
...
function Sparkline({
  ...
  return (
    ...
    >
      <path d={`M0 ${height - 4}H320`} stroke="var(--line)" strokeDasharray="3 4" />
      <polyline
        points={points}
        fill="none"
        stroke={color}
...
function ArchitectureCanvas({
  ...
  const canvasHeight = vertical ? t.nodes.length * 166 + 105 : rows > 1 ? rows * 175 + 110 : 330;
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const o = new ResizeObserver(([entry]) => setWidth(entry.contentRect.width));
    o.observe(el);
    return () => o.disconnect();
  }, []);
  ...
  const move = (e: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (g.id === 'canvas') setCamera({ x: g.origin.x + dx, y: g.origin.y + dy });
    else
      setPositions((p) => ({
        ...p,
    ...
    >
      ...
      >
        <svg
          ...
          width={Math.max(1800, right + 100)}
          height={Math.max(800, bottom + 100)}
          aria-hidden="true"
        >
          <defs>
            <marker
          ...
          const description =
            n.id === 'users'
              ...
              : n.id === 'app'
                ...
                : n.id === 'db'
                  ? `${view.architecture.replicas ?? 0} replicas · tier ${view.architecture.database + 1}`
                  : n.id === 'cache'
                    ? 'Read-through · 30s TTL'
                    : n.kind === 'replica'
                      ? `${view.architecture.replicas} read replicas`
                      : n.kind === 'db'
                        ? `Primary · tier ${view.architecture.database + 1}`
                        : n.id === 'standby'
                          ? `${view.architecture.instances} instances · warm standby`
                          : n.id === 'workers'
                            ? `${view.architecture.workers ?? 1} consumers`
                            : n.id === 'queue'
                              ? 'Buffered background jobs'
                              : n.id === 'stream'
                                ? `${view.architecture.stream} partitions`
                                : n.id === 'region'
          ...
          return (
            ...
            >
              ...
              <div className="node-measure">
                <span>
                  {n.id === 'users' ? 'Requests' : n.id === 'cache' ? 'Cache hits' : 'Utilization'}
                </span>
                <strong>
                  {n.id === 'users'
                    ? `${number(view.rps)}/s`
      ...
      <div className="canvas-bottom">
        <span>
          ...
          Request flow{' '}
          <span className="canvas-hint">{vertical ? '· Tap to inspect' : '· Drag to arrange'}</span>
        </span>
        {!vertical && (
...
function DecisionPanel({
  ...
  const node = topology(view.architecture, view.rps, view.workload).nodes.find(
    (n) => n.id === selected
  );
  const name = node?.label ?? selected;
  const failed = view.snapshot.activeFailures.some((f) => f.nodeId === selected);
  const Icon = icons[selected as keyof typeof icons] ?? Server;
  const component = selected.startsWith('db-') ? 'db' : selected === 'standby' ? 'app' : selected;
  const choices = DECISIONS.filter((d) =>
    d.components
      ? d.components.includes(component)
      : ['app', 'lb'].includes(component)
        ? ['app', 'horizontal'].includes(d.id)
        : ['db', 'cache', 'shards'].includes(component)
  ...
  return (
    <aside className="inspector">
      ...
      <div className="inspector-title">
        ...
        <span className={`status-dot ${(stats?.utilization ?? 0) > 0.85 ? 'amber' : ''}`} />
      </div>
      <div className="inspector-stats">
        <div>
          <span>{selected === 'users' ? 'Offered traffic' : 'Concurrency used'}</span>
          <strong>
            {selected === 'users' ? `${number(view.rps)}/s` : pct(stats?.utilization ?? 0)}
          </strong>
        </div>
        <div>
      ...
      <InfrastructureMetrics view={view} selected={selected} send={send} />
      {view.sandbox && <FailureLearning view={view} />}
      <div className={`diagnosis ${(stats?.queued ?? 0) > 4 ? 'warning' : ''}`}>
        ...
        <div>
          ...
          <p>
            {failed
              ? 'Inspect recovery and dependent services before adding capacity.'
              : (stats?.queued ?? 0) > 4
      ...
      <div className="decisions">
        {choices.map((d) => {
          ...
          return (
            <div className={`decision ${expanded === d.id ? 'expanded' : ''}`} key={d.id}>
              <button
              ...
              >
                <div>
                  <strong>
                    {d.title}
                    {!enabled && !prerequisite(view.architecture, d.id) && <Check size={13} />}
                  </strong>
              ...
              {expanded === d.id && (
                <div className="decision-body">
                  ...
                  <div className="decision-price">
                    <span>
                      <Clock3 size={13} />
                      {projectHours(view.architecture, d)}h engineering
                    </span>
                  ...
                  >
                    {!enabled
                      ? (prerequisite(view.architecture, d.id) ?? 'At limit / implemented')
                      : view.sandbox
                        ? 'Test in sandbox'
      ...
      <button className="secondary full" onClick={openCatalog}>
        Browse all {DECISIONS.length} tools
      </button>
...
function upgradePreview(v: View, d: Decision) {
  return upgrade(v.architecture, d.id);
}

export default function ScaleWorkbench({
  saveKey,
  scenario = 'saas',
  chooseProject,
}: {
  ...
  const { view, send, error, storage } = useSimulation(saveKey, scenario);
  ...
  useEffect(() => {
    const handle = (e: KeyboardEvent) => {
      if (
        (e.target as HTMLElement).closest('input,select,textarea,button,dialog,a,[contenteditable]')
      )
        return;
      if (e.code === 'Space') {
        e.preventDefault();
  ...
  const healthy = serviceHealthy(view.snapshot);
  const daily = (revenue(c) - cost(c.architecture) - engineeringCost(c)) / 30;
  const currentProject = DECISIONS.find((d) => d.id === c.project?.kind);
  const safeSelected = topology(view.architecture, view.rps, view.workload).nodes.some(
    (n) => n.id === selected
  )
    ? selected
    : 'db';
  return (
    <div className="app-shell">
      <header className="topbar">
        <Link className="brand" to="/play" aria-label="Back to Play">
          <div className="brand-mark">
            <i />
            <i />
            <i />
        ...
        <div className="workspace-name">
          ...
          <Link to="/play" className="return-to-play">
            <ArrowLeft size={14} /> Play
          </Link>
        </div>
      ...
      <nav className="sidebar" aria-label="Main navigation">
        <div className="nav-group">
          ...
          {(
            ...
            >
              <n.icon size={18} />
              <span>{n.label}</span>
              {n.id === 'projects' && c.project && <span className="nav-count">1</span>}
            </button>
          ))}
        </div>
        <div className="nav-group">
        ...
        <div className="sidebar-bottom">
          <div className="chapter-mini">
            ...
            <span>
              {companyProfile(c).difficulty}
              <strong>{companyProfile(c).title}</strong>
            </span>
          </div>
          <button className="nav-item" aria-label="Field guide" onClick={() => setGuide(true)}>
            <CircleHelp size={18} />
      ...
      <main id="main-content" className="main">
        <div className="page-heading">
              ...
              {view.sandbox
                ...
                : section === 'projects'
                  ...
                  : section === 'journal'
                    ...
                    : c.cash <= 0 || challengeStopped(c)
                      ? 'Your company is paused. Review the result and choose your next step below.'
                      : `Launch, grow to ${targetUsers(c).toLocaleString()} users, and sustain healthy, profitable service for 24 hours.`}
            </p>
          </div>
          <div className="time-control">
            ...
            <div className="transport">
              <button
                aria-label={view.paused ? 'Resume simulation' : 'Pause simulation'}
                disabled={!view.sandbox && (c.cash <= 0 || challengeStopped(c))}
                title={
                  !view.sandbox && (c.cash <= 0 || challengeStopped(c))
                    ? 'Review the run result below'
        ...
        {error && (
          <div className="alert error" role="alert">
            {error} <button onClick={() => window.location.reload()}>Reload saved company</button>
          </div>
        )}
        ...
        {view.offline && !dismissedOffline && (
          <div className="alert" role="status">
            <Clock3 size={18} />
            {view.offline}
            <button aria-label="Dismiss offline report" onClick={() => setDismissedOffline(true)}>
              <X size={16} />
            </button>
          </div>
        )}
        <section className="business-strip" aria-label="Company finances">
          <div>
            ...
            <small className={c.campaign && !healthy && !view.sandbox ? 'negative' : 'positive'}>
              {view.sandbox
                ? 'Company time frozen'
                : c.growth && healthy && !view.paused
                  ? `+${Math.round(companyProfile(c).growth * 100)}% organic / day`
                  : c.campaign && c.launched && !healthy && !view.paused
                    ? 'Customers leaving · service degraded'
          ...
          <div>
            ...
            <small>${companyProfile(c).revenuePerUser.toFixed(2)} per active user</small>
          </div>
          <div>
            <span>
              Infrastructure
        ...
        <CampaignPanel view={view} send={send} />
        {view.sandbox && (
          <div className="sandbox-banner">
            ...
            <div>
              ...
              <span>
                Company time is frozen. Measurements restart after each edit with the same seed.
              </span>
            </div>
            <button className="secondary" onClick={() => send({ type: 'return' })}>
              <ArrowLeft size={14} />
              Return to company
            </button>
          </div>
            ...
            <div className="workbench">
              <div className="architecture-main">
                ...
                <div
                  ...
                  role="region"
                  aria-label="Scrollable system architecture"
                >
                  <ArchitectureCanvas view={view} selected={safeSelected} select={setSelected} />
                </div>
                  ...
                  {view.sandbox ? (
                    <select
                      ...
                      onChange={(e) =>
                        send({
                          type: 'workload',
                          value: e.target.value as Workload,
                        })
                      }
                    >
                    ...
                    <button className="secondary small" onClick={() => send({ type: 'growth' })}>
                      {c.growth ? 'Hold growth' : 'Resume growth'}
                    </button>
                  )}
                </div>
                {!healthy && (
                  <p className="system-diagnosis">
                    {diagnosis({ ...c, workload: view.workload }, view.snapshot)}
                  </p>
                )}
                <div className="telemetry">
                  <div className="chart">
                    <div className="chart-heading">
                      <span>
                        <span className="legend-dot" />
                        Successful foreground throughput
                      </span>
                      <strong>
                        {number(s.goodputRps)}
                        <small> req/s</small>
                    ...
                    <Sparkline values={view.snapshot.history.map((h) => h.goodput)} />
                ...
                <details className="request-trace">
                  ...
                  <div className="trace-content">
                    <p>
                      One real request in this simulation, not an average. Waiting and work are
                      separate.
                    </p>
                    ...
                    {view.snapshot.trace && (
                      <p>
                        ...
                        Component p99 reports recorded work time. System p99 includes end-to-end
                        waiting.
                      </p>
                    )}
                  </div>
              ...
              <DecisionPanel
                ...
                send={send}
                openCatalog={() => setSection('projects')}
              />
            </div>
            {view.sandbox && view.baseline ? (
              <section className="comparison">
                <div>
                  ...
                  <p>
                    Baseline is the company snapshot when you branched. Changed workloads are
                    different experiments, not like-for-like comparisons.
                  </p>
                </div>
                <table>
                  <thead>
        ...
        {section === 'projects' && (
          <section className="projects-page">
            <div className="project-intro">
              ...
              <button className="primary" onClick={() => setSection('architecture')}>
                Back to architecture
                <ArrowRight size={16} />
              </button>
            </div>
            <ToolkitCatalog view={view} send={send} />
          </section>
        )}
        {section === 'journal' && (
            ...
            {c.events.map((e) => (
              <article key={e.id} className={`journal-entry ${e.tone}`}>
                <div className="journal-time">
                  ...
                  <span>{String(Math.floor(e.hour % 24)).padStart(2, '0')}:00</span>
                </div>
        ...
        {c.project && section !== 'projects' && (
          <button className="project-dock" onClick={() => setSection('projects')}>
            <GitBranch size={17} />
      ...
      >
        <div className="guide-head">
          ...
          <button className="icon-button" aria-label="Close guide" onClick={() => setGuide(false)}>
            <X size={19} />
          </button>
        </div>
        <h2 id="guide-title">
        ...
        <p>
          Traffic grows. Systems get busy. Your job is to understand the pressure and choose an
          intervention that makes sense.
        </p>
        <ol>
          <li>
            <strong>Observe.</strong> Select a component. Compare waiting requests, utilization, and
            measured latency.
          </li>
          <li>
            <strong>Experiment.</strong> Branch into Sandbox. Try upgrades or change the workload.
            Undo freely.
          </li>
          <li>
            <strong>Implement.</strong> Return to your company and start an engineering project.
            Watch what changes after deployment.
          </li>
        </ol>
        <div className="guide-facts">
          <h3>What the numbers mean</h3>
          <p>
            One real second advances one company hour at 1×. Every 168 company hours after launch, a
            12-hour primary app outage tests regional resilience. Request simulation has its own
            clock. Machine tiers map to concurrency, not hardware benchmarks. Prices and growth are
            fictional balancing values. Your current project: {companyProfile(c).title}.{' '}
            {companyProfile(c).lesson}
          </p>
          <p>
            Latency comes from simulated requests. The engine models queues, service time, cache hit
            probability and write contention. It does not reproduce MySQL, Redis, real queries,
            network stacks, or correctness guarantees. Cache eligibility and TTL tolerance are
            declared workload assumptions.
          </p>
          <p>
            Offline progress is capped at 24 company hours and stops at a service incident.
            Reopening reconstructs the request engine and unfinished background-job counts; it does
            not restore exact in-flight request timing or keys. Saves remain in this browser.
            Company time pauses in Sandbox.
          </p>
          <p>
            <a href="https://github.com/xevrion/breakscale" target="_blank" rel="noreferrer">
