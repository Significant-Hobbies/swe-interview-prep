...
const lb: ComponentBehaviour = {
  ...
  route: () => 'one',
  pickEdge: (ctx, state, req, out) => {
    if (state.config.partitionRouting && out.length) {
      const index = ctx.roll() < clamp01(state.config.hotKeyFraction) ? 0 : req.key % out.length;
      return out[index];
    }
    return ctx.pickWeightedOrLeastLoaded(out);
  },
};
