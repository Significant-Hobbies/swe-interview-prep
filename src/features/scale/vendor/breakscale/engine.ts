...
export class Engine implements BehaviourCtx {
  ...
  /** Scale save recovery: replay unfinished background jobs without counting new users. */
  restorePending(nodeId: string, count: number): void {
    const state = this.nodes.get(nodeId);
    if (!state || !['queue', 'streambroker'].includes(state.kind)) return;
    const total = Math.min(10000, Math.max(0, Math.floor(count)));
    const limit = state.config.queueLimit;
    if (state.kind === 'queue') state.config.queueLimit = Math.max(limit, total + state.waiting.length);
    for (let i = 0; i < total; i++) {
      const req = this.acquireReq();
      req.nodeId = nodeId;
      req.detached = true;
      req.key = Math.floor(this.rng.next() * KEYSPACE);
      req.rootStartMs = this.now;
      req.enterMs = this.now;
      if (state.kind === 'streambroker') {
        state.behaviour.onServiceComplete?.(this, state, req);
        this.resolve(req, true, 'error', 0);
      } else this.admit(state, req);
    }
    state.config.queueLimit = limit;
  }
