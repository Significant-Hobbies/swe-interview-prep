...
/**
 * The newest briefing, or nothing if the feed has gone stale.
 *
 * This used to return the first briefing unconditionally, so Today led with a
 * card headed "Start with the news" showing an item ten days old — the feed is
 * generated by a script that nothing schedules. An empty slot is more honest
 * than stale news presented as today's, and it makes the staleness visible
 * instead of quietly wrong.
 */
export function todayBriefing(now: Date = new Date()): LearningItem | undefined {
  const cutoff = now.getTime() - BRIEFING_MAX_AGE_DAYS * 24 * 60 * 60 * 1000;
  return LEARNING_SOURCES.items
    .filter((item) => item.sourceKind === 'briefing')
    .filter((item) => {
