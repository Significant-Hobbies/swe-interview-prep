...
slug: "how-a-socratic-coding-assistant-should-respond-to-learners"
target_query: "socratic ai coding assistant"
search_intent: "Informational: The reader is a developer or educator looking to understand how an AI should behave when its goal is to help humans learn to code, not just to write code for them."
meta_title: "How a Socratic coding assistant should respond to learners | SWE Learning OS"
meta_description: "An AI shouldn't just hand out code. Learn why Socratic AI coding assistants must probe understanding, withhold direct solutions, and enforce the Feynman Gate to build actual mastery."
---

# How a Socratic coding assistant should respond to learners

## Outline
1. Introduction: The distinction between generating code and teaching engineering
2. The core mechanism: No direct solutions
3. What probing understanding actually looks like
4. The Feynman Gate: Explaining causality back to the system
5. Evidence of learning vs. the illusion of competence
6. The integration of Socratic AI in a learning loop
7. Practical Next Action: Review your AI's system prompt
...
*   Link "learning loop" to the `/dashboard` or overarching SWE Learning OS concepts page to tie theoretical learning directly to practical application inside the app.
*   Link "spaced repetition algorithms" to the `ts-fsrs` or adaptive review feature page to highlight the platform's specific method of building durable memory.
*   Link "Feynman Gate" to internal documentation or blog posts detailing the explain-back requirement inside the Build Lab.
*   Link "low-level design patterns" to the LLD roadmaps or tracks available under `/learn`.

---

### Source Notes (Do Not Publish)
*   **Constraint Enforcement:** The uncompromising rule against direct solutions is documented in `docs/architecture/decisions/0005-socratic-no-solutions.md`, which states: "Socratic AI never gives direct solutions. CompanionPanel.tsx and the /api/chat Socratic system prompt never give direct solutions. They only probe understanding: ask what the user has tried, what they expect, what tradeoffs they're considering."
*   **Feynman Gate Integration:** The progression loop is detailed in `PROJECT_STATUS.md`: "Feynman Gate → FSRS progression (default flow): solving a drill triggers a skippable explain-back nudge; the AI-graded explanation maps onto per-concept FSRS ratings (feynmanRating), updates mastery, then surfaces a 'next weakest concept' card (BuildLab) so the loop closes: drill → explain → mastery update → next weakest concept."
*   **Artifact Requirement:** The core product principle "Require an artifact, decision, prediction, or other inspectable evidence" is defined in `PRODUCT.md`.
*   **Limitations:** The product does not support multi-tenant learning workflows or team administration (`PRODUCT.md`), and the deployed Socratic AI stream is currently restricted to owner authentication (`PROJECT_STATUS.md`). There are no fabricated commercial usage claims or customer testimonials included in this draft, adhering to the brand commitments in `PRODUCT.md`.