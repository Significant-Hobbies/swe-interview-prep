...
slug: "applying-fsrs-spaced-repetition-to-software-engineering-knowledge"
target_query: "spaced repetition software engineering"
search_intent: "Informational: The reader wants to understand how to practically apply spaced repetition algorithms, specifically FSRS, to complex engineering topics rather than simple vocabulary flashcards."
meta_title: "Applying FSRS Spaced Repetition to Software Engineering Knowledge"
meta_description: "Explore how to use the FSRS algorithm for software engineering knowledge. Learn how an evidence-gated learning loop moves spaced repetition beyond rote memorization to systemic understanding."
---

## Outline
1. **The Gap in Engineering Knowledge Retention**: Why flashcards fail for system design and code architecture.
2. **Introducing FSRS for Complex Domains**: How the Free Spaced Repetition Scheduler algorithm differs from legacy systems like SM-2.
3. **The Evidence-Gated Learning Loop**: Combining Concept, Drill, Build, Review, and Apply.
4. **Concrete Implementation**: Mapping Feynman-style explanations and artifact creation to FSRS mastery ratings.
5. **Trade-offs and Limitations**: What FSRS cannot do and when deep practice must supersede scheduled review.
6. **Next Action**: How to incorporate evidence-gated repetition into your own engineering workflows.
...
- **Learning Roadmaps**: Link to `/learning` or specific roadmap pages like `/learning/system-design` when discussing curriculum topography.
- **Practice and Playgrounds**: Link to `/practice` and `/playground` when describing the environment where artifacts (code, diagrams) are generated.
- **Dashboard Priorities**: Link to `/dashboard` when discussing how the system answers "what should I learn today?".

### Source notes (Internal/Non-publishable)
- **Claims Supported by Repository Evidence**:
  - **FSRS Implementation**: `src/lib/fsrs.ts` imports from `ts-fsrs`, sets parameters (`request_retention: 0.9`), maps states (`MasteryRow`, `MasteryRating`), and implements `reviewConcept`. It shares `masteryConfidence` with the server via `shared/lib/confidence.mjs`.
  - **Evidence-Gated Learning Loop / No learning without an artifact**: `docs/product/overview.md` ("No learning without an artifact... Every concept maps to drills and an artifact... The Playground (Monaco + Excalidraw + Socratic AI + Feynman Gate) is the build/drill workspace").
  - **Feynman Gate to FSRS Rating**: `src/lib/feynmanRating.ts` demonstrates the exact mapping from a 0-100 grade to FSRS `MasteryRating` ('again', 'hard', 'good', 'easy'). It explicitly caps ratings at 'hard' when gaps are identified ("A named gap caps the concept at 'hard'").
  - **Dashboard Priority**: `docs/product/overview.md` states: "Failed practice recovery outranks overdue retention, which outranks roadmap progression. The winning concept is paired with an explicit evidence contract."
  - **Curriculum Scope**: `docs/product/overview.md` and `PROJECT_STATUS.md` describe the 259 canonical concepts, 19 tracks, and 26 roadmaps, supporting the limitation that total tracked concepts are in the hundreds, not thousands.
- **Limitations**:
  - The article refrains from mentioning commercial metrics, user volumes, or any synthetic testimonials as dictated by the marketing guidelines.
  - The distinction between guest state in browser and durable state via Cloudflare D1 (as seen in `PROJECT_STATUS.md` and `docs/product/overview.md`) is abstracted out in favor of focusing on the methodology.
