...
function checkDuplication() {
  const outputDirectory = mkdtempSync(join(tmpdir(), 'swe-interview-prep-jscpd-'));
  run('pnpm', [
    'exec',
    'jscpd',
    ...productionPaths,
...
function checkDependencies() {
  ...
  console.log(
    `Dependencies: ${critical} critical, ${high} high, ${unexpected.length} unexpected; ` +
      `${severe.length - unexpected.length} accepted transitive advisories.`
  );
...
function checkSuppressions() {
  const files = ['src', 'api', 'handlers', 'shared', 'functions', 'scripts', 'tests'].flatMap(
    (root) => sourceFiles(resolve(projectRoot, root))
  );
  const matches = files
    .filter(
      (file) => file !== currentFile && !relative(projectRoot, file).startsWith('src/data/library/')
    )
    .flatMap((file) =>
