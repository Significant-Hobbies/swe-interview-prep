...
function usefulForEngineers(r) { return Math.abs(r) > 0.5 ? 'screening' : 'weak'; }`,
  ...
  'design-ab-test-power': designCase(['primaryMetric', 'sampleSize', 'duration', 'guardrails', 'peekingPolicy']),
  'interpret-srm-alert': [{
...
console.log(pi[1].toFixed(2), simulate(P, 1000).toFixed(1).length >= 3 ? 'ok' : 'bad');`,
  ...
  'design-honest-backtest': designCase(['trainHoldoutSplit', 'benchmark', 'transactionCosts', 'limitations']),
  'ml-bpe-merge-step': [{
...
const STAR_IDS = [
  'star-leadership', 'star-conflict', 'star-tough-decision', 'star-cross-team',
  'star-failure', 'star-communication', 'star-prioritization', 'star-innovation',
  'star-customer-feedback', 'star-ownership',
];
