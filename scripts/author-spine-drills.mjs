...
const NEW_DRILLS = [
  // --- ML spine (19) ---
  {
    ...
    testCases: [
      {
        setup:
          'function dot(a,b){return a.reduce((s,x,i)=>s+x*b[i],0);} function matmulShape(a,b){return [a.length,b[0].length];}',
...
function xent(logits,y){const p=softmax(logits);return -Math.log(p[y]);}`,
  ...
  },
  {
    ...
    testCases: [
      {
        setup:
          'function l2Step(w,g,lr,d){return w-lr*(g+d*w);} function adamwStep(w,g,lr,d){return (w-lr*g)*(1-lr*d);}',
...
function scaled(T,xs){return softmax(xs.map(x=>x/T));}`,
  ...
  },
  {
    ...
    hints: [
      'Even dims sin, odd dims cos in original paper.',
  ...
  },
  {
    ...
    expectedOutput:
      'Wrong labels/shapes, train-eval leakage, broken tokenizer, masked tokens all padding, etc.',
    hints: [
      'Karpathy recipe: overfit one batch first.',
      'If you cannot overfit one batch, data or loss is wrong.',
    ],
  ...
  },
  {
    ...
    expectedOutput:
      'Optimizer moments reset — loss spike / different trajectory; effective warmup replay.',
    hints: [
      'Adam stores m and v per parameter.',
      'Resume needs weights + optimizer + step + RNG if exact replay.',
    ],
  ...
  },
  {
    ...
    hints: [
      'tokens = docs × tokens/doc × epochs.',
      'Scale data with model size for compute-optimal training.',
    ],
  ...
  },
  {
    ...
    expectedOutput:
      'LM loss improved token distribution but task metric insensitive or needs few-shot/prompt — check task-specific eval.',
...
function reverse(head){let p=null,c=head;while(c){const n=c.n;c.n=p;p=c;c=n;}return p;}`,
  ...
  },
  {
    ...
    expectedOutput:
      'startsWith true; search true; search("ba") true prefix but search("ban") false.',
...
const GATE_TEST_CASES = {
  'construct-confidence-interval': [
    {
      setup:
        'function ci95(phat,n,z=1.96){const se=Math.sqrt(phat*(1-phat)/n);return [phat-z*se,phat+z*se].map(x=>+x.toFixed(3));}',
  ...
  'standard-error-mean': [
    {
      setup:
        'function seMean(s,n){return s/Math.sqrt(n);} function ci(xbar,se,z=1.96){return [xbar-z*se,xbar+z*se].map(v=>+v.toFixed(2));}',
  ...
  'compute-gradient-2d': [
    {
      setup:
        'function grad(x,y){return [2*x+3*y,3*x+2*y];} function norm(v){return Math.sqrt(v[0]*v[0]+v[1]*v[1]);}',
  ...
  'compute-sharpe-drawdown': [
    {
      setup:
        'function annSharpe(mean,sigma,rf=0){return (mean*252-rf)/(sigma*Math.sqrt(252));} function maxDD(peak,trough){return (trough-peak)/peak;}',
  ...
  'type-errors-power': [
    {
      setup:
        'function errorType(h0True,rejected){if(h0True&&rejected)return "I";if(!h0True&&!rejected)return "II";return "ok";}',
...
console.log(
  `Added ${added} editorial drills, ${testCasesAdded} gate testCase sets, linked ${linked} concepts.`
);
